...
public class BetaCaveCarver extends Carver<ProbabilityConfig> {
    ...
    protected void carveTunnels(
        ...
        int mainChunkZ,
        double x, double y, double z, 
        float tunnelSysWidth, float f1, float f2, 
        int branch, int branchCount, 
        ...
        for(; branch < branchCount; branch++)
        {
            ...
            if(!noStarts && branch == randBranch && tunnelSysWidth > 1.0F) {
                carveTunnels(chunk, rand, mainChunkX, mainChunkZ, x, y, z, random.nextFloat() * 0.5F + 0.5F, f1 - 1.570796F, f2 / 3F, branch, branchCount, 1.0D);
                carveTunnels(chunk, rand, mainChunkX, mainChunkZ, x, y, z, random.nextFloat() * 0.5F + 0.5F, f1 + 1.570796F, f2 / 3F, branch, branchCount, 1.0D);
                return;
            }
            ...
            carveRegion(chunk, null, 0, 64, mainChunkX, mainChunkZ, x, y, z, yaw, pitch, null); // Don't move this into above if statement, breaks.
    ...
    protected boolean carveRegion(Chunk chunk, Function<BlockPos, Biome> posToBiome, long seed, int seaLevel,
            int mainChunkX, int mainChunkZ, double x, double y, double z, double yaw, double pitch, BitSet carvingMask) {
        ...
        int minX = MathHelper.floor(x - yaw) - mainChunkX * 16 - 1; // Get min and max extents of tunnel, relative to chunk coords
        ...
        // System.out.println("z: " + z + ", mainChunkZ: " + mainChunkZ + ", minZ: " + minZ);
        ...
        if (super.isRegionUncarvable(chunk, mainChunkX, mainChunkZ, minX, maxX, minY, maxY, minZ, maxZ)) { // Use vanilla methods, for now.
            return false;
        }
            ...
            for(int relZ = minZ; relZ < maxZ; relZ++) {
                ...
                for(int relY = maxY - 1; relY >= minY; relY--) {
                    ...
                    if (isPositionExcluded(scaledRelX, scaledRelY, scaledRelZ, -1)) {
                        ...
                        //Block blockAbove = chunk.getBlockState(new BlockPos(relX, setY + 1, relZ)).getBlock();
                        
                        if(block == Blocks.GRASS_BLOCK) {
                            isGrassBlock = true;
                        }
                        
                        if(block == Blocks.STONE || block == Blocks.DIRT || block == Blocks.GRASS_BLOCK) { // Not using canCarveBlock for accuracy,
                                                                                                           // CHANGE IF NECESSARY, e.g. for cave biomes!
                            if(relY < 10) { // Set lava below y = 10
                                ...
                                // I believe this replaces carved-out dirt with grass, if block that was removed was grass.
                                if(isGrassBlock && chunk.getBlockState(new BlockPos(relX, setY - 1, relZ)).getBlock() == Blocks.DIRT) {
                                    chunk.setBlockState(blockPos.set(relX, setY - 1, relZ), Blocks.GRASS_BLOCK.getDefaultState(), false);
                                }
                            }
                        }
                    }
    ...
    protected boolean isPositionExcluded(double scaledRelativeX, double scaledRelativeY, double scaledRelativeZ, int y) {
        return scaledRelativeY > -0.69999999999999996D &&
            scaledRelativeX * scaledRelativeX + scaledRelativeY * scaledRelativeY + scaledRelativeZ * scaledRelativeZ < 1.0D;
    }
