...
public final class BetaVoronoiGeneratorType extends GeneratorType {
    public static final GeneratorType INSTANCE = new BetaVoronoiGeneratorType();
    
    public static final OldGeneratorSettings betaSettings = new OldGeneratorSettings(new CompoundTag(), false);
        ...
        new ImmutableMap.Builder<Optional<GeneratorType>, ScreenProvider>()
            ...
            .put(
                Optional.<GeneratorType>of(INSTANCE), (createWorldScreen, generatorSettings) -> {
                    return new CustomizeAlphaLevelScreen(createWorldScreen, betaSettings);
                }
            )
            .build();

    private BetaVoronoiGeneratorType() {
        super("beta_voronoi");
    }
    ...
    protected ChunkGenerator getChunkGenerator(Registry<Biome> biomes, Registry<ChunkGeneratorSettings> genSettings, long seed) {
        ...
        betaSettings.settings = OldGeneratorSettings.createInfSettings(WorldType.BETA.getName(), BiomeType.BETA_VORONOI.getName(), ModernBeta.BETA_CONFIG.generateOceans);
        return new NoiseChunkGenerator(new OldBiomeSource(seed, biomes, betaSettings.settings), seed, () -> betaSettings.wrapped);
    }
