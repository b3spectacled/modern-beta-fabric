...
public class BetaChunkGenerator extends NoiseChunkGenerator {
    ...
    private void setStructureStart(ConfiguredStructureFeature<?, ?> configuredStructureFeature, DynamicRegistryManager dynamicRegistryManager, StructureAccessor structureAccessor, Chunk chunk, StructureManager structureManager, long long7, ChunkPos chunkPos, Biome biome) {
        StructureStart<?> structureStart11 = structureAccessor.getStructureStart(ChunkSectionPos.from(chunk.getPos(), 0), configuredStructureFeature.feature, chunk);
        int integer12 = (structureStart11 != null) ? structureStart11.getReferences() : 0;
        
        //StructureConfig structureConfig13 = this.structuresConfig.getForType(configuredStructureFeature.feature);
        StructureConfig structureConfig13 = this.settings.wrapped.getStructuresConfig().getForType(configuredStructureFeature.feature);
        
        if (structureConfig13 != null) {
            StructureStart<?> structureStart14 = configuredStructureFeature.tryPlaceStart(dynamicRegistryManager, this, this.biomeSource, structureManager, long7, chunkPos, biome, integer12, structureConfig13);
            structureAccessor.setStructureStart(ChunkSectionPos.from(chunk.getPos(), 0), configuredStructureFeature.feature, structureStart14, chunk);
        }
    }
    ...
    private void buildBetaSurface(Chunk chunk) {
        ...
        sandNoise = beachNoiseOctaves.generateNoiseOctaves(sandNoise, chunkX * 16, chunkZ * 16, 0.0D, 16, 16, 1,  eighth, eighth, 1.0D);
        gravelNoise = beachNoiseOctaves.generateNoiseOctaves(gravelNoise, chunkX * 16, 109.0134D, chunkZ * 16, 16, 1, 16, eighth, 1.0D, eighth);
        stoneNoise = stoneNoiseOctaves.generateNoiseOctaves(stoneNoise, chunkX * 16, chunkZ * 16, 0.0D, 16, 16, 1, eighth * 2D, eighth * 2D, eighth * 2D);
