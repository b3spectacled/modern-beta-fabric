...
public class SingleBiomeProvider extends AbstractBiomeProvider {
    ...
    private Identifier loadLegacySettings(CompoundTag settings) {
        ...
        if (oldWorldType.equals("alpha")) {
            if (biomeType == BiomeType.CLASSIC || biomeType == BiomeType.PLUS) biomeId = ClassicBiomes.ALPHA_ID;
            if (biomeType == BiomeType.WINTER) biomeId = ClassicBiomes.ALPHA_WINTER_ID;
        } else if (oldWorldType.equals("infdev")) {
            if (biomeType == BiomeType.CLASSIC || biomeType == BiomeType.PLUS) biomeId = ClassicBiomes.INFDEV_415_ID;
            if (biomeType == BiomeType.WINTER) biomeId = ClassicBiomes.INFDEV_415_WINTER_ID;
        } else if (oldWorldType.equals("alpha")) {
            if (biomeType == BiomeType.CLASSIC || biomeType == BiomeType.PLUS) biomeId = ClassicBiomes.INFDEV_227_ID;
            if (biomeType == BiomeType.WINTER) biomeId = ClassicBiomes.INFDEV_227_WINTER_ID;
        }
