    ...
    private void generateFloatBorder(Chunk chunk) {
        ...
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                ...
                chunk.setBlockState(mutable.set(x, 1, z), this.levelTheme == IndevTheme.HELL ? BlockStates.LAVA : BlockStates.WATER, false); // not sure whether WATER at the bottom has a great gameplay/survival experience (you got fall damage in water in the indev version)
            }
        }
    }
